// Script para adicionar summaries a todos os modelos
const fs = require('fs');
...
console.log('🔧 Adicionando summaries a todos os modelos...');
...
// Para cada modelo, adicionar summary
let updatedCount = 0;

for (const [modelId, summary] of Object.entries(summaries)) {
  // Procurar pelo modelo usando um padrão mais específico
  ...
  if (matches && matches.length > 0) {
    content = content.replace(modelPattern, (match) => {
      updatedCount++;
      return match + `,\n      summary: '${summary}'`;
    });
  }
...
console.log(`✅ Adicionado summaries a ${updatedCount} modelos`);