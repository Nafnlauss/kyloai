// Script corrigido para adicionar summaries
const fs = require('fs');
...
console.log('🔧 Adicionando summaries aos modelos...');

// Função para adicionar summary após credits
function addSummaryAfterCredits(content, modelId, summary) {
  // Procurar o padrão: id: 'modelId' ... credits: { ... }
  const regex = new RegExp(
    `(id:\\s*['"]${modelId}['"][\\s\\S]*?credits:\\s*\\{[^}]+\\})`,
    'g'
  );
  
  return content.replace(regex, (match) => {
    // Verificar se já tem summary
...
const modelSummaries = [
  // Luma (já tem, mas vamos verificar)
...
// Aplicar cada summary
for (const [modelId, summary] of modelSummaries) {
  const originalContent = content;
  content = addSummaryAfterCredits(content, modelId, summary);
  
  if (content !== originalContent) {
    updatedCount++;
    console.log(`✓ Adicionado summary para: ${modelId}`);
  }
}
...
console.log(`\n✅ Total: ${updatedCount} summaries adicionadas`);